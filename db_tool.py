...
class DBTool():
    def init(self, db_file):
        self.db = Database(sqlite3.connect(db_file, check_same_thread=False))
    ...
    def verify_email_code(self, email):
        err_msg = ""
        print("self.user_map:", self.user_map)
        if email in self.user_map.keys():
            err_msg = "Email has been registered"
            return False, err_msg
        subject = "Verification Code from Voice Training Service"
        verification_code = str(random.randint(100000,999999))
        body = "Your verification code is " + verification_code
        sender = "voicetraining@163.com"
        password = "XGINPJYCOSKIVWMQ"
    ...
    def login(self, email, password):
        ...
        if email in self.user_map:
            if password == self.user_map["email"]:
                return True, err_msg
    ...
    def update_password(self, email, old_password, new_password):
        if email not in self.user_map.keys():
        ...
        try:
            ...
            self.user_table.update(row_id, {"password": new_password})
